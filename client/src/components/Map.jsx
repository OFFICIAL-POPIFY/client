...
const Test = () => {
  const KakaoMap = ({ latitude, longitude }) => {
    ...
    console.log(`검색되는지 보기 위함1  lat: ${latitude} lon: ${longitude}`);

    useEffect(() => {
      kakao.maps.load(() => {
        console.log("kakao map load");
  ...
  const { container, places, pagination } = KakaoMap({
    latitude: lat,
    longitude: lon,
  });
  ...
  const searchPlaces = (keyword) => {
    KakaoMap({ latitude: lat, longitude: lon });
  };
